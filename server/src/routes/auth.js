...
router.post('/register', function(req, res) {
  ...
  var existing = queryOne('SELECT id FROM users WHERE email = ?', [email]);
  if (existing) {
  ...
  var id = gid();
  execSQL('INSERT INTO users (id, email, password_hash, nickname) VALUES (?, ?, ?, ?)',
...
router.get('/me', authMiddleware, function(req, res) {
  var user = queryOne('SELECT id, email, nickname, created_at FROM users WHERE id = ?', [req.userId]);
